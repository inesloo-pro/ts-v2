...
type ListeningSubItem = 'competitors' | 'industry' | 'hashtags' | 'feeds';
...
const listeningSubItems: { id: ListeningSubItem; label: string }[] = [
...
export function TopBar({ activeMenuItem, onMenuItemChange }: TopBarProps) {
  ...
  const [activeListeningSubItem, setActiveListeningSubItem] = useState<ListeningSubItem>('competitors');
  const listeningRef = useRef<HTMLDivElement>(null);

  // Close dropdown when clicking outside
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (listeningRef.current && !listeningRef.current.contains(e.target as Node)) {
  ...
  const isListeningActive = activeMenuItem === 'listening';
      ...
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center justify-between px-[12px] relative size-full">
          ...
          <div className="content-stretch flex gap-[12px] h-full items-center relative shrink-0">
            ...
            {/* Listening — with dropdown */}
            <div ref={listeningRef} className="h-full relative shrink-0">
              <button
                onClick={() => {
                  onMenuItemChange('listening');
                  setListeningOpen((v) => !v);
                }}
                className="h-full relative shrink-0 cursor-pointer bg-transparent border-none"
              >
                {isListeningActive && (
                ...
                <div className="content-stretch flex items-center justify-between pl-[4px] pr-[8px] py-[4px] relative size-full">
                  <div className="content-stretch flex gap-[8px] items-center relative shrink-0">
                    ...
                    <div className="relative shrink-0 size-[16px]">
                      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
                        <path d="M8 1.5A5.5 5.5 0 0 0 2.5 7v1.5H4A1.5 1.5 0 0 1 5.5 10v2A1.5 1.5 0 0 1 4 13.5H3A1.5 1.5 0 0 1 1.5 12V7a6.5 6.5 0 0 1 13 0v5A1.5 1.5 0 0 1 13 13.5h-1A1.5 1.5 0 0 1 10.5 12v-2A1.5 1.5 0 0 1 12 8.5h1.5V7A5.5 5.5 0 0 0 8 1.5Z" fill="#76869A"/>
                      </svg>
                    </div>
                    <p className="font-['Gilroy:Semibold',sans-serif] leading-[14px] not-italic relative shrink-0 text-[#76869a] text-[12px] tracking-[-0.072px] whitespace-nowrap">
                      Listening
                    ...
                    <div className="relative shrink-0 size-[10px]">
                      <div className="absolute inset-[33.75%_21.25%_34.79%_21.25%]">
                        <svg
                          className={`absolute block inset-0 size-full transition-transform duration-150 ${listeningOpen ? 'rotate-180' : ''}`}
                          fill="none"
                          preserveAspectRatio="none"
                          viewBox="0 0 6.9 3.77574"
                        >
              ...
              {/* Dropdown */}
              {listeningOpen && (
                <div className="absolute left-0 top-[calc(100%+4px)] z-50 bg-white rounded-[10px] shadow-[0_4px_16px_rgba(0,0,0,0.12)] border border-[rgba(0,0,0,0.08)] py-[6px] min-w-[180px]">
                  {listeningSubItems.map((sub) => (
                    <button
                      key={sub.id}
                      onClick={() => {
                        setActiveListeningSubItem(sub.id);
                        setListeningOpen(false);
                      }}
                      className={`w-full text-left px-[14px] py-[8px] flex items-center gap-[8px] cursor-pointer transition-colors hover:bg-[rgba(0,0,0,0.04)] bg-transparent border-none ${
                        activeListeningSubItem === sub.id ? 'text-[#1d1d1b]' : 'text-[#76869a]'
                      }`}
                    >
                      {activeListeningSubItem === sub.id && (
                        <div className="w-[3px] h-[3px] rounded-full bg-[#76869a] shrink-0" />
                      )}
                      {activeListeningSubItem !== sub.id && (
                        <div className="w-[3px] h-[3px] shrink-0" />
                      )}
                      <p className={`font-['Gilroy:Semibold',sans-serif] text-[12px] tracking-[-0.072px] leading-[14px] whitespace-nowrap ${
                        activeListeningSubItem === sub.id
                          ? 'font-[\'Gilroy:Bold\',sans-serif] text-[#1d1d1b]'
                          : ''
                      }`}>
                        {sub.label}
                      </p>
